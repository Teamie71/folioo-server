...
export function Modal({ open, onClose, children, panelClassName = 'max-w-md' }) {
    ...
    return html`
        ...
        >
            <div
                class=${`bg-white rounded-2xl shadow-2xl ${panelClassName} w-[90%] max-h-[85vh] overflow-y-auto p-6
...
export function GrantEventRewardModal({ open, user, onClose, onSuccess }) {
    ...
    const formView = html`
        <${React.Fragment}>
            ...
            <div
                class="grid grid-cols-1 xl:grid-cols-[minmax(0,1.15fr)_minmax(300px,0.85fr)] gap-4 items-start"
            >
                <!-- Left: Event details -->
                                  ...
                                  <div class="space-y-2">
                                      <div class="flex items-center gap-3">
                                          ...
                                          <div class="flex items-center gap-2">
                                              <button
                                                  ...
                                                  onClick=${() =>
                                                      setCsExperienceQty(
                                                          Math.max(0, csExperienceQty - 1)
                                                      )}
                                                  class="w-7 h-7 flex items-center justify-center border border-gray-300
                                                       rounded-md text-gray-500 hover:bg-gray-100 text-sm font-bold"
                                              >
                                                  −
                                              </button>
                                              <span
                                                  class="w-6 text-center text-sm font-semibold tabular-nums"
                                                  >${csExperienceQty}</span
                                              >
                                              <button
                                                  type="button"
                                                  onClick=${() =>
                                                      setCsExperienceQty(
                                                          Math.min(10, csExperienceQty + 1)
                                                      )}
                                                  class="w-7 h-7 flex items-center justify-center border border-gray-300
                                                       rounded-md text-gray-500 hover:bg-gray-100 text-sm font-bold"
                                              >
                                      ...
                                      <div class="flex items-center gap-3">
                                          ...
                                          <div class="flex items-center gap-2">
                                              ...
                                              <span
                                                  class="w-6 text-center text-sm font-semibold tabular-nums"
                                                  >${csCorrectionQty}</span
                                              >
                                              <button
                                                  type="button"
                                                  onClick=${() =>
                                                      setCsCorrectionQty(
                                                          Math.min(10, csCorrectionQty + 1)
                                                      )}
                                                  class="w-7 h-7 flex items-center justify-center border border-gray-300
                                                       rounded-md text-gray-500 hover:bg-gray-100 text-sm font-bold"
                                              >
                ...
                <div class="rounded-2xl border border-amber-100 bg-white p-4 space-y-4">
                    ...
                    <div>
                        ...
                        >
                        <div
                            class="w-full border border-gray-200 bg-gray-50 rounded-lg px-3 py-2.5 text-sm text-gray-700"
                        >
                            ${selectedEvent?.title || '-'}
                        </div>
                    </div>
            ...
            <div class="bg-gray-50 rounded-xl p-4 space-y-2 text-sm mb-5">
                ...
                <div class="flex justify-between">
                    ...
                    <span class="font-semibold text-amber-600"
                        >${selectedEvent?.title || eventCode} (${eventCode})</span
                    >
                </div>
                <div class="flex justify-between">
