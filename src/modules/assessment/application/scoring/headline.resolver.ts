...
export function resolveTopTrait(traitVector: TraitVector): TraitKind {
    let top = ALL_TRAIT_KINDS[0];
    let topScore = traitVector[top];
    for (const trait of ALL_TRAIT_KINDS.slice(1)) {
        if (traitVector[trait] > topScore) {
            top = trait;
            topScore = traitVector[trait];
        }
    }
    return top;
}
