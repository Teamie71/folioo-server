...
export class ExternalPortfolioFacade {
    ...
    async getExternalPortfolios(correctionId: number): Promise<StructuredPortfolioResDto[]> {
        ...
        const portfolioIds =
            await this.portfolioCorrectionService.findPortfolioIdsByCorrectionId(correctionId);
    ...
    async createExternalPortfolioBlock(
        ...
        const currentCount =
            await this.portfolioCorrectionService.countItemsByCorrectionId(correctionId);
