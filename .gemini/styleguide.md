...
5. **Facade에 비즈니스 로직 포함**: Facade가 순수 조율 역할을 넘어 비즈니스 계산/검증을 직접 수행하는 경우
6. **비즈니스 상수 위치**: 도메인 비즈니스 규칙(제한 값 등)이 엔티티가 아닌 서비스에 정의된 경우
7. **모듈 등록 오류**: 타 도메인의 Repository/Entity가 잘못된 모듈에 등록된 경우
