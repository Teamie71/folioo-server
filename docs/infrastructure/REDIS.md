...
| `UPSTASH_REDIS_URL`   | X    | -           | Upstash Redis URL (Upstash 사용 시)      |
| `UPSTASH_REDIS_TOKEN` | X    | -           | Upstash Redis 토큰 (Upstash 사용 시)     |
...
현재는 `IoRedisCacheAdapter` 구현체를 바인딩합니다. 추후 Upstash 적용 시 Adapter만 교체하면 됩니다.
...
현재 구조는 다음 계층으로 구성됩니다.

1. `RedisService` (애플리케이션에서 사용하는 진입점)
2. `CachePort` (추상화)
3. `IoRedisCacheAdapter` (구현체, `ioredis` 의존)

Upstash를 도입할 때는 `UpstashCacheAdapter`를 추가하고 `CachePort` 바인딩만 변경하면 됩니다.

```text
Service -> RedisService -> CachePort -> IoRedisCacheAdapter(ioredis)
```

```text
Service -> RedisService -> CachePort -> UpstashCacheAdapter(upstash)
```
